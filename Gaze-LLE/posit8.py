...
def float_to_posit8(x):
    # Simulation: clip and cast as float8-like dynamic (real Posit8 will have more advanced logic)
    ...
    scaled = ((x + max_abs) / (2*max_abs) * 255).round()
    scaled = torch.clamp(scaled, 0, 255)
    return scaled.to(torch.uint8)

def posit8_to_float(p):
    max_abs = 16
    f = (p.float() / 255) * (2*max_abs) - max_abs
    return f

def quantize_tensor_to_posit8(tensor):
    # Tensor to simulated posit8
    quantized = float_to_posit8(tensor)
    return quantized

def dequantize_tensor_from_posit8(quantized):
    return posit8_to_float(quantized)

class Posit8LinearLayer(nn.Module):
    def __init__(self, original_linear):
        super().__init__()
        self.in_features = original_linear.in_features
        self.out_features = original_linear.out_features
        # Quantize weight and bias
        self.register_buffer('quantized_weight', quantize_tensor_to_posit8(original_linear.weight.data))
        if original_linear.bias is not None:
            self.register_buffer('quantized_bias', quantize_tensor_to_posit8(original_linear.bias.data))
        else:
            self.quantized_bias = None

    def forward(self, input_tensor):
        weight = dequantize_tensor_from_posit8(self.quantized_weight)
        bias = dequantize_tensor_from_posit8(self.quantized_bias) if self.quantized_bias is not None else None
        return F.linear(input_tensor, weight, bias)

class Posit8Conv2dLayer(nn.Module):
    def __init__(self, original_conv):
        ...
        self.register_buffer('quantized_weight', quantize_tensor_to_posit8(original_conv.weight.data))
        if original_conv.bias is not None:
            self.register_buffer('quantized_bias', quantize_tensor_to_posit8(original_conv.bias.data))
        else:
            self.quantized_bias = None
...
def quantize_model_to_posit8(model):
    quantized_layers = []
...
def analyze_model_memory_usage(model, name="Model", quant_type=None):
    ...
    for buffer_name, buffer in model.named_buffers():
        # Quantized buffers by type
        if 'quantized_weight' in buffer_name or 'quantized_bias' in buffer_name:
            quantized_parameters += buffer.numel()
            # Size per entry (bits): FP4=0.5, FP8/Posit8=1
            if quant_type == "FP4":
                total_size_bytes += buffer.numel() * 0.5
            else: # FP8 or Posit8
                total_size_bytes += buffer.numel() * 1.0
        else:
...
def compare_quantized_outputs(orig, posit8):
    for k in set(orig) & set(posit8):
        print(f"Output Key: {k}")
        base = orig[k]
        mse_posit8 = torch.mean((base - posit8[k])**2).item()
        print(f"Posit8 MSE: {mse_posit8:.6f}")
...
def full_quantization_comparison_pipeline():
    ...
    model_posit8 = copy.deepcopy(model)
    print("Applying Posit8 Quantization...")
    quantize_model_to_posit8(model_posit8)

    print("\n--- Model Size and Memory Usage ---")
    analyze_model_memory_usage(model, "Original Model")
    analyze_model_memory_usage(model_posit8, "Posit8 Model", "Posit8")

    # Compression ratios
    _, _, orig_mb   = analyze_model_memory_usage(model, "O", None)
    _, _, posit8_mb = analyze_model_memory_usage(model_posit8, "P", "Posit8")
    print(f"Posit8 Compression: {orig_mb/posit8_mb:.2f}x")

    print("\n--- Benchmark Inference ---")
    benchmark_inference(model, 100)
    benchmark_inference(model_posit8, 100)
    ...
    out_posit8 = get_output(model_posit8)
    compare_quantized_outputs(out_orig, out_posit8)
